...
typedef struct {
    ...
    XAxis_fp_example_uio_map maps[ MAX_UIO_MAPS ];
} XAxis_fp_example_uio_info;

/***************** Variable Definitions **************************************/
static XAxis_fp_example_uio_info uio_info;
...
static int uio_info_read_name(XAxis_fp_example_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XAxis_fp_example_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XAxis_fp_example_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XAxis_fp_example_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XAxis_fp_example_Initialize(XAxis_fp_example *InstancePtr, const char* InstanceName) {
	XAxis_fp_example_uio_info *InfoPtr = &uio_info;
